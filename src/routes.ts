...
apiRoutes.get("/download/:key", async (c) => {
	...
	const r2Object = await c.env.AUDIO_BUCKET.get(key);
...
apiRoutes.post(
	...
	async (ctx) => {
		...
		const res = await ctx.env.AUDIO_BUCKET.put(
			key,
...
apiRoutes.get("/load/:key", async (ctx) => {
	...
	const r2object = await ctx.env.AUDIO_BUCKET.get(key, {
		range,
	});
